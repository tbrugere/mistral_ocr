...
async def upload_file_content(client: Mistral, file_name: str, file: BufferedReader|IO[bytes], purpose: Literal["ocr", "batch"]="ocr") -> AsyncIterator[UploadFileOut]:
    ...
    try: 
        ...
        try:
            ...
            logging.error("Couldn't delete file from mistralai cloud, you should delete it manually", exc_info=e)
    return 

@contextlib.asynccontextmanager
async def upload_file_to_url(client: Mistral, file_path: Path) -> AsyncIterator[FileSignedURL]:
    with file_path.open("rb") as f:
        async with upload_file_content(client, file_path.name, f, purpose="ocr") as upload_response:
            signed_url = await client.files.get_signed_url_async(file_id=upload_response.id, expiry=3)
            yield signed_url 

@contextlib.asynccontextmanager
async def upload_files(client: Mistral, file_paths: Iterable[Path]) -> AsyncIterator[Sequence[FileSignedURL]]:
    async with contextlib.AsyncExitStack() as stack:
        with click.progressbar(file_paths, label="uploading files") as bar:
            ...
            file_uris = [await stack.enter_async_context(upload_file_to_url(client, path)) for path in bar]
            yield file_uris
...
async def wait_for_job_to_finish(client: Mistral, job: mistralai.BatchJobOut, n_files: int):
    last_known_done = 0
...
@click.command()
@click.argument("file_path", type=click.Path(exists=True, dir_okay=False, path_type=Path, readable=True), nargs=-1 )
@click.option("--api-key", envvar="MISTRAL_API_KEY", required=True, help="API key for Mistral OCR service.", prompt=True, hide_input=True)
@click.option("--model", type=str, default="mistral-ocr-latest")
@click.option("--output-file", "-o", type=click.File("wb", ), required=False, help="save the raw result of the batch job")
@click.option("--resume", type=str, required=False, help="resume from job id")
def sync_main(file_path: tuple[Path, ...], api_key: str, model: str, output_file: BinaryIO|None = None, resume: str|None = None, ) -> None:
...
async def main(api_key: str, file_path: tuple[Path, ...], ocr_model: str, output_file: BinaryIO|None = None, resume: str|None = None) -> None:
    ...
    if len(name_to_path) != len(file_path):
        raise ValueError("For now we don't support uploading several files with the same name")
    async with contextlib.AsyncExitStack() as stack:
        if not resume: 
            file_uris = await stack.enter_async_context(upload_files(client, file_path))
            batch_file = stack.enter_context(create_batch_file(file_uris, [p.name for p in file_path]))
            batch_data_url = await stack.enter_async_context(upload_file_content(client, "batch_file", batch_file, purpose="batch"))
            job = client.batch.jobs.create(
                input_files=[batch_data_url.id], 
                model=ocr_model, 
                endpoint="/v1/ocr"
                    )
        ...
        output_results = [ResponseModel.model_validate_json(j) async for j in output_results_json]
        ...
        for result in output_results:
            ...
            result.response.write(
                    output_path_dir, 
                    output_path_dir.with_suffix(".md").name
            )
